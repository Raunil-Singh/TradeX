...
void Listener::listener()
{
    while (true)
    {
        ...
        if (identity_seq_num != last_seq_num + msg_per_packet)
        {
            ...
            {
                last_seq_num += 23;
                queue.push(last_seq_num); // sequence number. Change retransmitter to send back a whole packet
            }
        }
...
void Listener::missing_packet_request()
{
    ...
    while (true)
    {
        ...
        //open tcp connection
        int status = connect(sockfd_tcp, (struct sock_addr*)&addr_tcp, sizeof(addr_tcp));
        if (status < 0)
        {
            switch(errno){
                //handle errors
