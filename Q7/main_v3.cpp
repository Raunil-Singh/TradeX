...
void randomThreadSimulator(){
    ...
    for(int i = 0; i < 1'000'000; i++){
        order_count.fetch_add(intDistr(rGen), std::memory_order_seq_cst);
        trade_count.fetch_add(intDistr(rGen), std::memory_order_seq_cst);
        cancel_count.fetch_add(intDistr(rGen), std::memory_order_seq_cst);        
    }
}
