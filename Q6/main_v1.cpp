...
void mergeSort(int level, std::vector<int>& vector, int startIndex, int endIndex, int depth){
    ...
    if(depth >= level){
        ...
        depth++;
        std::thread threadLeft(std::thread(mergeSort, level, std::ref(vector), startIndex, startIndex + elementCount/2 - 1, depth));
        std::thread threadRight(std::thread(mergeSort, level, std::ref(vector), startIndex + elementCount/2, endIndex, depth));
        threadLeft.join();
        threadRight.join();
...
int main(int argc, char* argv[]){
    ...
    std::thread mergeSortThread(mergeSort, level, std::ref(vector), 0, VECTOR_SIZE - 1, 0);
